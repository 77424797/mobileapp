 ...
 */
define(['zepto.min.js', 'clip/iscroll-zoom.js', 'clip/hammer.js', 'clip/lrz.all.bundle.js'], function(require, exports, module) {
	var defaultOption = {
		size: [260, 260],
		outputSize: [0, 0],
		outputType: "jpg",
	...
	function clipImage(container, option) {
		...
		var $container, // 容器，包含裁剪视图层和遮罩层
			...
			$rotateLayer, // 旋转层
			$view, // 最终截图后呈现的视图容器
			canvas = document.createElement("canvas"), // 图片裁剪用到的画布
			hammerManager,
			...
			$(file).on("change", function() {
				...
				if(!/image\/\w+/.test(files.type)) {
					...
					fileReader.onload = function(e) {
						lrz(files, lrzOption).then(function(rst) {
		...
		$(ok).click(function() { //裁剪图片
			if(!imgLoaded) {
				console.log("亲，当前没有图片可以裁剪!");
			} else {
		...
		function resetScroll() {
			...
			refreshScroll(imgWidth, imgHeight,true);
		...
		function refreshScroll(width, height,flag) {
			$moveLayer.css({
				"width": width,
				"height": height
			});
		...
		function globalToLoacl($layer, x, y) { // 计算相对于窗口的x、y坐标在$layer上的坐标
			...
			return {
				x: x + $(window).scrollLeft() - layerOffset.left,
				y: y + $(window).scrollTop() - layerOffset.top
			};
		}
		...
		function createImg(src) {
			if($img) {
				$rotateLayer.html('');// 删除旧的图片以释放内存，防止IOS设备的webview崩溃
			}
			$img = $("<img>").css({"user-select": "none","pointer-events": "none"});
		...
		function init() {
			...
			var $mask = $("<div class='photo-clip-mask'>").css({
				...
				"width": clipWidth,
				"height": clipHeight,
				'border':$container.height()+'px solid rgba(0, 0, 0, 0.498039)',
				'top':-($container.height()+clipHeight)/2+'px',
				'left':-(($container.height()*2+clipHeight)-$container.width())/2+'px',
				"pointer-events": "none"
			}).appendTo($container);
			var $mask_view=$('<div></div>').css({
			...
			if(circle){
				$mask_view.css('border-radius','50%');
				$mask.css('border-radius','50%');
			}
			// 初始化视图容器
			$view = $(view);
			if($view.length) {
	...
	var exports = {
		clipImage: function(container, option) {
			clipImage(container, option);
		}
	};
